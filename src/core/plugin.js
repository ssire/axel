...
(function ($axel) {
  ...
  ///////////////////////////////////////////////////////
  function PluginFactory( name, spec, klass ) {
  this.type = name;
  this.spec = spec;
  this.klass = klass;
  }
  ...
  PluginFactory.prototype = {
   ...
   createModel : function createModel (aContainer, aXTUse, aDocument) {
     var h = this.klass.generator(aContainer, aXTUse, aDocument); // invoke klass markup generator
   ...
   createEditorFromTree : function createEditorFromTree (aHandleNode, aXTUse, aDocument) {
     var f, inst = new this.klass();
     inst._init(aHandleNode, aDocument, createUniqueKey(this.type));
     inst._parseFromTemplate(aXTUse);
     // optional "filterable" feature 
     if (this.spec.filterable) {
       f = inst.getParam('filter'); // filters chain
       if (f) {
         inst = this.applyFilters(inst, f);
       }
     }
  ...
  $axel.plugin.register = function ( name, spec, defaults, genfunction, klassdefs ) {
   ...
   if (xtiger.editor.Plugin.prototype.pluginEditors[name]) {
     ...
     klass.generator = genfunction; // static markup generator function
   
     // dynamically create plugin factory class
     factoklass = new PluginFactory(name, spec, klass);
     ...
     klass.prototype.onInit = klassdefs.onInit;
     klass.prototype.onAwake = klassdefs.onAwake;
     klass.prototype.onLoad = klassdefs.onLoad;
     klass.prototype.onSave = klassdefs.onSave;

     // overwrite basic plugin methods
     // FIXME: check method exists before overwriting to print warnings
     $axel.extend(klass.prototype, klassdefs.api, false, true);

     // add specific methods
     // FIXME: check method does not exist to print warnings
     $axel.extend(klass.prototype, klassdefs.methods);
