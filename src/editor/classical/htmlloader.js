 ...
 */
xtiger.editor.HTMLLoader = function () {
}

xtiger.editor.HTMLLoader.prototype = {
  ...
  loadDataIter : function (n, dataSrc, stack) { 
    var name, doit = false, cname, point;
    if (n.nodeType == xtdom.ELEMENT_NODE) {
      cname = n.getAttribute('class');
      if ((! cname) || ((cname.indexOf('xtt-') == -1) && (cname.indexOf('xtiger-') == -1)) ||
       (cname.indexOf('axel-repeat-unset') != -1)) {
        doit = true;                                                                         
      }
    ...
    if (doit) {
        ...
        point = stack[ stack.length - 1 ];
        point = dataSrc.getVectorFor(name, point);
        if ((point instanceof Array) && (dataSrc.lengthFor(point) > 0)) {
          stack.push(point); // one level deeper
        } else {
          doit = false;
        }
        this._loadDataIter(n, dataSrc, stack);
        if (doit) {
          stack.pop ();       
        }
        // this.queue.pop();
...
// xtiger.editor.Generator.prototype.defaultLoader = new xtiger.editor.HTMLLoader ();
